...
foreach ($oAccounts in $Accounts){
    ...
    New-ADUser -Name $oAccounts.Name -samAccountName $oAccounts.samAccountName -Description $oAccounts.Description -Path ("ou=Service Accounts,OU=Tier 0,OU=Admin," + $($sDSE)) -AccountPassword (ConvertTo-SecureString $oPWD -AsPlainText -force) -Enabled $true -PasswordNeverExpires $true

    If ($oAccounts.Membership -ne ""){
        Add-Log -LogEntry("Adding " + $oAccounts.Name + " to " + $oAccounts.Membership)
        Add-ADPrincipalGroupMembership -Identity $oAccounts.samAccountName -MemberOf $oAccounts.Membership
    }
    $error.Clear()

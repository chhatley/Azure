<#
    ...
    .DESCRIPTION
        v1.19 19 Jun 2018 - Fixed bugs with linking GPOs. Added Backup-Computers and Restore-Computers functions.
        v1.18 17 Jun 2018 - Added SkipADMX and SkipRoleDefinitions to the script
        V1.17 15 Nov 2017 - Added LAPS WriteComputerAdmPwd function to enable SELF on computer objects to update it.
        V1.16 15 Oct 2017 - Generates a random password when importing users.User passwords must be reset.(new generate-PWD function and changes at around line 3000 to call the function.)
        V1.15 12 Oct 2017 - Updated for use in domains where Administrator account has been renamed 
        V1.14 12 Sep 2017 - Updated for 1703 Baseline GPO Name changes.
		V1.13 30 Aug 2017 - updated to add the external powershell linking commands into the main script as switches
        V1.12 20 July 2017 - Updated the import GPO code to use GPO name, instead of the GUID - no longer rely on the manifest.xml file for import GPOs. Added code to unhide hidden GPO backup XML files.
        V1.11 07 Dec 2016 - Added ACL fixes for DomainJoinComputers and ForcePasswordChangeAtLogon functions. Add new ResetComputerAdmPwd function.
        V1.10 13 Oct 2016 - Added Append to ExportWMI code to allow export of multiple WMIQueries, added code to remove duplicate WMIFilters
...
param(
    ...
    [switch] $Link2012R2,
    [switch] $ImportADMXs,
    [switch] $LinkDomainPolicies,
    [switch] $RedirectComputersContainers,
    [switch] $skipADMXs,
    [switch] $skipRoleDefinitions    
)
...
Function Backup-Computers($path)
{
   ...
   foreach($OU in $OUs)
   {
       ...
       $computerObjects = Get-ADComputer -LDAPFilter '(name=*)' -SearchBase $OU.distinguishedName -Properties DistinguishedName,Name,SamAccountName -SearchScope OneLevel
       ...
       if ($computer.DistinguishedName -like '*Domain Controllers*')
            ...
            {
            $computerObjectElement = $configXML.CreateElement("Computer")
            $computerObjectElement.SetAttribute("Name", $computer.Name)

            $computerObjectDN = $configXML.CreateElement("distinguishedName")
            #$userObjectDN.Set_InnerText($user.distinguishedName)
            $DN = $computer.distinguishedName -replace((([regex]::matches(($computer.distinguishedName),"(?i)DC=[^,]*") | % {$_.value}) -join ","),"")
            $computerObjectDN.Set_InnerText($DN)
            $computerObjectElement.AppendChild($computerObjectDN) | out-null
                                  

            $computerObjectSAM = $configXML.CreateElement("SamAccountName")
            $computerObjectSAM.Set_InnerText($computer.SamAccountName)
            $computerObjectElement.AppendChild($computerObjectSAM) | out-null
                                      
            $configXML.Configuration["Computers"].AppendChild($computerObjectElement) | out-null

            Write-Log -Msg ("INFORMATION: Backed up computer " + $computer.name) -category "Info"                  
        }
       }
    }
...
Function Restore-Computer($strComputerName, $strComputerSamAccountName, $strComputerDescription, $strComputerPath)
{
    $error.Clear()
    #check if the user already exists...
    $Result = [bool]([adsisearcher]"samaccountname=$strComputerSamAccountName").FindOne()

    If($Result)
    {
        Write-Host -ForegroundColor DarkGray ("INFORMATION: Computer already exists: " + $strComputerName)
    }
    else
    {
        Write-Host -ForegroundColor Green ("INFORMATION: Creating the Computer: " + $strComputerName)
        try
        {            
           New-ADComputer -Name $strComputerName -SamAccountName $strComputerSamAccountName -Description $strComputerDescription -Path $strComputerPath    
        }
        catch
        {
           If ($error -match "computer already exists")
           {
                Write-Host -ForegroundColor Cyan ("INFORMATION: The computer" + $strComputerName + " already exists. Skipping")
           }
           Else
           {
               Write-Host -ForegroundColor Red ("ERROR: Unable to create the computer " + $strComputerName + $strComputerPrincipalName)
        ...
        If(!$error)
        {
           Write-Host -ForegroundColor Green ("INFORMATION: Successfully created the computer " + $strComputerName)
        }
    }
} 
...
{  
    Process {
        ...
        $AdminPWD = ([System.Web.Security.Membership]::GeneratePassword(22,2)) # 22 bytes long: 
        Return $AdminPWD
    }
}
Function AdminAccountChecker
{
#
#      .Description
#         Checks the default domain administrator account and determine if it has been renamed.
#         Then update the migrationtable with this account.
#
#      Determining the account name
$DomainSID = (Get-ADDomain).domainsid
$DomainAdmin = $DomainSID.ToString() + "-500"
$identityvalues = Get-ADUser -Identity $DomainAdmin
$DomainAdminName = $identityvalues.name

If ($DomainAdminName -ne "Administrator") {
write-host "Domain Admin Name is $DomainAdminName"
write-host "Backing up default migration table"
Copy-Item "migrationtable.migtable" "migrationtable.backup"
$MigTableDomainAdminName = $DomainAdminName+"@"
[string]$MigTablePath = ".\MigrationTable.migtable" 
(get-content $MigTablePath) | foreach-object {$_ -replace "Administrator@", $MigTableDomainAdminName} | set-content $MigTablePath
#$MigTableFile = Get-Content -Path $MigTablePath -ErrorAction stop
}



}
Function Import-ADMXs($ADMXBackupFolder)
{
#
#      .Description
#         Imports the ADMXs into AD
#
# Copy ADMX and ADML to DC policy storage

Copy-Item "$ADMXBackupFolder\ADMXs\EMET.admx" "C:\Windows\PolicyDefinitions"
Copy-Item "$ADMXBackupFolder\ADMXs\EMET.adml" "C:\Windows\PolicyDefinitions\en-us"
Write-Host -ForegroundColor Green ("INFORMATION: EMET templates copied ")
Copy-Item "$ADMXBackupFolder\ADMXs\FEP2010.admx" "C:\Windows\PolicyDefinitions"
Copy-Item "$ADMXBackupFolder\ADMXs\FEP2010.adml" "C:\Windows\PolicyDefinitions\en-us"
Write-Host -ForegroundColor Green ("INFORMATION: FEP 2010 templates copied ")
Copy-Item "$ADMXBackupFolder\ADMXs\admpwd.admx" "C:\Windows\PolicyDefinitions"
Copy-Item "$ADMXBackupFolder\ADMXs\admpwd.adml" "C:\Windows\PolicyDefinitions\en-us"
Write-Host -ForegroundColor Green ("INFORMATION: admpwd templates copied ")
Copy-Item "$ADMXBackupFolder\ADMXs\MSS-legacy.admx" "C:\Windows\PolicyDefinitions"
Copy-Item "$ADMXBackupFolder\ADMXs\MSS-legacy.adml" "C:\Windows\PolicyDefinitions\en-us"
Write-Host -ForegroundColor Green ("INFORMATION: MSS-legacy templates copied ")
Copy-Item "$ADMXBackupFolder\ADMXs\SecGuide.admx" "C:\Windows\PolicyDefinitions"
Copy-Item "$ADMXBackupFolder\ADMXs\SecGuide.adml" "C:\Windows\PolicyDefinitions\en-us"
Write-Host -ForegroundColor Green ("INFORMATION: SecGuide templates copied ")
Copy-Item "$ADMXBackupFolder\ADMXs\UserProxySettings.admx" "C:\Windows\PolicyDefinitions"
Copy-Item "$ADMXBackupFolder\ADMXs\UserProxySettings.adml" "C:\Windows\PolicyDefinitions\en-us"
Write-Host -ForegroundColor Green ("INFORMATION: UserProxySettings templates copied ")
Write-Host -ForegroundColor Green ("INFORMATION: All templates copied ")
}

Function Link2016DCBaselines
{
...
Write-host -ForegroundColor Green ("INFORMATION: *- Tier 0 DCs SCM Windows Server 2016 - Domain Controller Baseline -Linked")
...
Write-Host -ForegroundColor Green ("INFORMATION: *- Tier 0 DCs MSFT Windows 10 and Server 2016 Defender -Linked")
Set-GPLink -Name "*- Tier 0 DCs MSFT Internet Explorer 11 - Computer" -Target "ou=Domain Controllers,$DomainDN" -LinkEnabled Yes
Write-Host -ForegroundColor Green ("INFORMATION: *- Tier 0 DCs MSFT Internet Explorer 11 - Computer -Linked")
Set-GPLink -Name "*- Tier 0 DCs EMET Baseline" -Target "ou=Domain Controllers,$DomainDN" -LinkEnabled Yes
Write-host -ForegroundColor Green ("INFORMATION: *- Tier 0 DCs EMET Baseline -Linked")
Remove-GPO -Name "*- Tier 0 DCs MSFT Windows Server 2012 R2 Domain Controller Baseline"
Write-host -ForegroundColor Green ("INFORMATION: *- Tier 0 DCs MSFT Windows Server 2012 R2 Domain Controller Baseline - Removed")
Remove-GPO -Name "*- WMI Filter Placeholder GPO - Not in Use"
Write-host -ForegroundColor Green ("*- WMI Filter Placeholder GPO - Not in Use -Removed")
Write-Host -ForegroundColor Green ("INFORMATION: Configuration Complete ")
}

Function Link2012R2DCBaselines
{
$DomainDN = get-ADDomain |select -ExpandProperty DistinguishedName
...
Write-Host -ForegroundColor Green ("INFORMATION: *- MSFT Windows Server 2012 R2 Domain Controller Baseline -Linked")
Set-GPLink -Name "*- Tier 0 DCs EMET Baseline" -Target "ou=Domain Controllers,$DomainDN" -LinkEnabled Yes
Write-Host -ForegroundColor Green ("INFORMATION: *- Tier 0 DCs EMET Baseline -Linked")
Remove-GPLink -Name "*- Tier 0 DCs MSFT Internet Explorer 11 - Computer" -Target "ou=Domain Controllers,$DomainDN"
Write-host -ForegroundColor Green ("INFORMATION: *- Tier 0 DCs MSFT Internet Explorer 11 - Computer -Unlinked")
Remove-GPLink -Name "*- Tier 0 DCs MSFT Windows 10 and Server 2016 - Defender" -Target "ou=Domain Controllers,$DomainDN"
Write-host -ForegroundColor Green ("INFORMATION: *- Tier 0 DCs MSFT Windows 10 and Server 2016 - Defender")
Remove-GPO -Name "*- Tier 0 DCs SCM Windows Server 2016 - Domain Controller Baseline"
Write-host -ForegroundColor Green ("INFORMATION: *- Tier 0 DCs SCM Windows Server 2016 - Domain Controller Baseline - Removed")
Remove-GPO -Name "*- WMI Filter Placeholder GPO - Not in Use"
Write-host -ForegroundColor Green ("INFORMATION: *- WMI Filter Placeholder GPO - Not in Use - Removed")

Write-Host -ForegroundColor Green ("INFORMATION: Configuration Complete ")
}

Function LinkDomainPolicies
{
$DomainDN = get-ADDomain |select -ExpandProperty DistinguishedName

Set-GPLink -Name "*- Additional Domain Security Settings" -Target $DomainDN -LinkEnabled Yes
Write-Host -ForegroundColor Green ("INFORMATION: *- Additional Domain Security Settings - linked at domain level ")
Set-GPLink -Name "*- MSFT Windows 10 and Server 2016 - Domain Security" -Target $DomainDN -LinkEnabled Yes
Write-Host -ForegroundColor Green ("INFORMATION: *- MSFT Windows 10 and Server 2016 - Domain Security - linked at domain level ")
Write-Host -ForegroundColor Green ("INFORMATION: Configuration Complete ")

}

#START Computer Redirection Functions
...
Write-Host -ForegroundColor Green ("Computer Containers Redirected")
}

#    END Computer Redirection Functions
...
Param(
    ...
    Get-ADObject -Server $SrcServer -SearchBase $WMIPath -Filter {objectClass -eq 'msWMI-Som'} -Properties msWMI-Author, msWMI-Name, msWMI-Parm1, msWMI-Parm2 |
     ...
     Export-CSV (Join-Path $Path WMIFilters-temp.csv) -Append -NoTypeInformation
}
...
#    
	...
	#unhiding all files in the GPO folder
	Get-ChildItem $path -File -Recurse -Force | where { $_.Attributes -match "Hidden"} | % {$_.Attributes = 'Normal'}
}
        ...
        If(((Get-GPO -Name $gpoName.Name -ErrorAction SilentlyContinue) -and ($OverwriteExistingPolicies -eq $True)) -or (!(Get-GPO -name $gpoName.name -ErrorAction SilentlyContinue)))
        {
            ...
            {       
                ...
                {                    
                    ...
                    Import-GPO -BackupGpoName $gpoName.Name -TargetName $gpoName.Name -Path $gpoPath -MigrationTable $migTable -CreateIfNeeded | Out-Null
                }
                Catch
                {        
                    #Write-Host -ForegroundColor Red ("ERROR: Unable to import the policy " + ($backupGuid.GPODisplayName."#cdata-section"))
                    Write-Host -ForegroundColor Red ("ERROR: Unable to import the policy " + $gpoName.Name)
            ...
            Else
            {
                try
                {
                    ...
                    Import-GPO -BackupGpoName $gpoName.Name -TargetName $gpoName.Name -Path $gpoPath -CreateIfNeeded | Out-Null
            ...
            {       
                 ...
                 Write-Host -ForegroundColor Green ("INFORMATION: Restored the policy " + $gpoName.Name)
...
Function WriteComputerAdmPwd($objGroup, $objOU, $inheritanceType)
{
$error.Clear()
...
# The schema must be extended for LAPS
$objAcl.AddAccessRule((New-Object System.DirectoryServices.ActiveDirectoryAccessRule $groupSID,"WriteProperty,ReadProperty","Allow",$guidmap["ms-Mcs-AdmPwd"],$inheritanceType,$guidmap["computer"]))
...
{
    Write-Host -ForegroundColor Red ("ERROR: Unable to grant the group " + $objGroup.Name + " permissions to write local administrator password on OU" + $objOU)
}
If(!$error)
{
    Write-Host -ForegroundColor Green ("INFORMATION: Granted the group " + $objGroup.Name + " permissions to write local administrator password on OU " + $objOU)
}
...
#  
    ...
    { 
        ...
        if ($BackupPolicies){
            ...
            CreateMigTable
            #Update WMIFilters file and remove duplicates
            Import-Csv (Join-Path $gpoFolder GPO\WMIFilters-temp.csv) | sort msWMI-Name –Unique | Export-CSV (Join-Path $gpoFolder GPO\WMIFilters.csv) -NoTypeInformation
            Remove-Item (Join-Path $gpoFolder GPO\WMIFilters-temp.csv) 
        }
   }   
   ElseIf($Restore)
   {    
        ...
        If($restoreUsers -eq $true)
        {
            ...
            #If you want all the passwords of your restored users to be the same uncomment the line below and change "P1nkGr@peFru1t" to be your password of choice.
            #$userPassword=(ConvertTo-SecureString "P1nkGr@peFru1t" -AsPlainText -force)
            Foreach ($user in $configXML.Configuration.Users.User)
            {
            #Comment out the two lines below if uncommenting the password line above.
            $restuserpassword = (Generate-PWD)
            $userPassword=(ConvertTo-SecureString $restuserpassword -AsPlainText -force)
       ...
       #END
       # Link 2016 Domain Controller Baselines
       If ($Link2016 -eq $True)
       {
       Write-Host -ForegroundColor DarkGray "INFORMATION: Linking Windows Server 2016 Domain Controller Baselines"
       Link2016DCBaselines
       }
       #END
       If ($Link2012R2 -eq $True)
       {
       Write-Host -ForegroundColor DarkGray "INFORMATION: Linking Windows Server 2012 R2 Domain Controller Baselines"
       Link2012R2DCBaselines
       }
       #END
       # Import ADMXs
       If ($ImportADMXs -eq $True)
       {
       Write-Host -ForegroundColor DarkGray "INFORMATION: Copying ADMXs to AD Central Storage"
       Import-ADMXs($BackupFolder)
       }
       #END 
       # Link Domain Policies
       If ($LinkDomainPolicies -eq $True)
       {
       Write-Host -ForegroundColor DarkGray "INFORMATION: Linking Domain level policies"
       LinkDomainPolicies
       } 
       #END
       # Redirect Computers to Computer Quarantine.
       If ($RedirectComputersContainers -eq $True)
       {
       write-host -ForegroundColor DarkGray "INFORMATION: Redirecting The Computers Container to Computer Quarantine"
       RedirectComputers
       }
       #End
   }
   Write-Host -ForegroundColor Green ("ExportImport Complete")   
}
...
Function CheckConfig
{
    ...
    } elseif ($Restore)
    {
        ...
        Else
        {
         ...
         If ($Link2016 -eq $true -and $Link2012R2 -eq $true)
        { 
            Write-Host -ForegroundColor Red "ERROR: You cannot link both 2012 R2 and 2016 DC baselines - it's one or the other"
            Exit
        } 
    ...
    if ($Restore){
        ...
        Write-Host -ForegroundColor Cyan ("Link Domain Controller 2016 policies: " + $Link2016)
        Write-Host -ForegroundColor Cyan ("Link Domain Controller 2012 R2 policies: " + $Link2012R2)
        Write-Host -ForegroundColor Cyan ("Link Domain Policies: " + $LinkDomainPolicies)
        Write-Host -ForegroundColor Cyan ("Redirect Computers Container to Computer Quarantine: " + $RedirectComputersContainers)
